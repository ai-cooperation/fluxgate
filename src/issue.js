// 登入會員領 MCP key（綁 hub uid，之後用 key 時讀 live tier）。需 Firebase ID token。
...
export async function issueMcpKey(request, env) {
  ...
  const u = bearer ? await verifyIdToken(bearer) : null;
  ...
  await env.KV.put(`key:${key}`, JSON.stringify({
    tier, uid: u.uid, email: u.email, label: u.email, via: "hub", created: new Date().toISOString(),
  }));
